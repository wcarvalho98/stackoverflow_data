...
O dataset possui **`r nrow(dataset)`** linhas e **`r ncol(dataset)`** colunas. Os dados são dispostos da seguinte forma:
```{r warning=FALSE, results='asis'}
library(knitr)
kable(head(dataset)) %>% kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
res = dataset %>% filter(colSums(!is.na(df)) > 0)
nrow(res)
ncol(res)
```
