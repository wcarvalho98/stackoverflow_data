...
```{r message=FALSE, warning=FALSE}
firstData <- select(dataset, MainBranch, Country, Currency, CompTotal, CompFreq) %>% drop_na()
```
...
Como podemos observar, existem alguns tipos diferentes de educação e,
para isso iremos quantificar cada tipo de educação para os 10 paises que
possuem mais desenvolvedores da seguinte maneira:
...
dev_by_country = dev_by_country %>% arrange(desc(Devs)) %>% slice_head(n = 10)

res = data.frame(matrix(nrow = nrow(dev_by_country), ncol = length(education_levels)))
rownames(res) = dev_by_country$Country
colnames(res) = education_levels

for (row in rownames(res)) {
  for (col in colnames(res)) {
    res[row, col] = nrow(education_country %>% filter(Country == row, EdLevel == col))
  }
}

final_result = data.frame(matrix(nrow = nrow(res) * ncol(res), ncol = 3))
colnames(final_result) = c("Country", "EdLevel", "Devs")

for (row in 1:nrow(res)) {
  for (col in 1:ncol(res)) {
    actualRow = col + (row - 1) * ncol(res)
    final_result[actualRow, "Country"] = rownames(res)[row]
    final_result[actualRow, "EdLevel"] = colnames(res)[col]
    final_result[actualRow, "Devs"] = res[row, col]
  }
}
```

Após prepararmos os dados e armazená-los é suficiente a amostragem dos
dados por meio de uma tabela reprensentando os 10 países que tem mais
desenvolvedores e seu nível de educação:

```{r message=FALSE, warning=FALSE, results='asis'}
ggplot(final_result, aes(x=Country, y=Devs, group=EdLevel)) + 
  geom_line(aes(linetype=EdLevel, color=EdLevel)) + geom_point(aes(color=EdLevel)) + theme_minimal() +
...
# Comparativo entre homens e mulheres como desenvolvedores

Nesta seção iremos fazer um comparativo entre homens e mulheres no mundo do desenvolvimento. Para isso iremos tratar o nosso dataset de maneira a remover todos as respostas inválidas (NA). Em seguida podemos extrair diversos dados como a quantidade total de pessoas que declararam seu sexo, a quantidade de homens, mulheres, trans homens e trans mulheres assim como homens e mulheres transexuais da seguinte forma:

```{r message=FALSE, warning=FALSE}
gender_dataset = dataset %>% filter(!is.na(Gender)) %>% select(Gender, Trans, Sexuality) %>% drop_na()
...
A partir desses dados podemos representar esses dados de uma forma visual, como podemos ver no seguinte gráfico:

```{r message=FALSE, warning=FALSE}
ggplot(gender_dataset, aes(x=gender_dataset$Gender, y=gender_dataset$Sexuality))
```