...
Nessa seção iremos fazer um comparativo do nível educacional dos desenvolvedores baseado em seu país de nascença. Para isso teremos que coletar as colunas "EdLevel" e "Country" as quais correspondem, respectivamente, ao nível de educação e o país de origem do desenvolvedor.
O seguinte código ira gerar um subset com a educação de cada desenvolvedor da base e uma correlação com seu país de origem:
...
Como podemos observar, existem alguns tipos diferentes de educação e, para isso iremos quantificar cada tipo de educação para os 10 paises que possuem mais desenvolvedores da seguinte maneira:
